...
public class OrderDAOImpl implements OrderDAO {
	...
	public boolean placeOrder(Order order) {
		...
		try (Connection con = DbConnection.getDatabaseConnection()) {
			...
			PreparedStatement pst = con.prepareStatement(
					"insert into productorder" + "(orderdunits,address,ordereddate,requesteddate,accepted,cancelled,"
							+ "confirmed,billamount,customerid,productid) " + "values(?,?,?,?,?,?,?,?,?,?)");
	...
	public int[] getProductDetailsFromOrderId(Order o) {
		...
		try (Connection con = DbConnection.getDatabaseConnection()) {
			PreparedStatement pst = con.prepareStatement("select productid,orderunits from productorder where orderid=?");
			...
			if (rs.isBeforeFirst()) {
				while (rs.next()) {
					details[0]=rs.getInt("productid");
					details[1]=rs.getInt("orderunits");
				}
			}
